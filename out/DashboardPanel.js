...
class DashboardPanel {
    static createOrShow(extensionUri, handlers = {}) {
        ...
        const panel = vscode.window.createWebviewPanel("serialDashboard", "Serial Dash", column || vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        DashboardPanel.currentPanel = new DashboardPanel(panel, extensionUri, handlers);
    }
    constructor(panel, extensionUri, handlers) {
        this._disposables = [];
        this._panel = panel;
    ...
    _getHtmlForWebview(webview) {
        const scriptUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "dashboard.js")));
        const styleUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "style.css")));
        // Chart.js and Canvas-Gauges from CDN
        const chartJsUri = "https://cdn.jsdelivr.net/npm/chart.js";
        const canvasGaugesUri = "https://cdn.rawgit.com/Mikhus/canvas-gauges/master/gauge.min.js";
        // Inline SVG icons (stroke uses currentColor so they inherit button text color)
        const icon = (path) => `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;
        const iconPlus = icon('<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>');
        const iconTrash = icon('<polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/>');
        const iconPause = icon('<rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/>');
        const iconDownload = icon('<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>');
        ...
        let brandLogo = "";
        try {
            const svgPath = path.join(this._extensionUri.fsPath, "assets", "icons", "icon.svg");
            brandLogo = fs
        ...
        const iconSerial = brandLogo ||
            icon('<rect x="2" y="7" width="20" height="14" rx="2"/><path d="M6 7V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2"/><circle cx="8" cy="14" r="1" fill="currentColor"/><circle cx="12" cy="14" r="1" fill="currentColor"/><circle cx="16" cy="14" r="1" fill="currentColor"/>');
        const iconFullscreen = icon('<path d="M3 9V4a1 1 0 0 1 1-1h5"/><path d="M21 9V4a1 1 0 0 0-1-1h-5"/><path d="M3 15v5a1 1 0 0 0 1 1h5"/><path d="M21 15v5a1 1 0 0 1-1 1h-5"/>');
        const iconExitFullscreen = icon('<path d="M9 3H4a1 1 0 0 0-1 1v5"/><path d="M15 3h5a1 1 0 0 1 1 1v5"/><path d="M9 21H4a1 1 0 0 1-1-1v-5"/><path d="M15 21h5a1 1 0 0 0 1-1v-5"/>');
        const iconPlug = icon('<path d="M12 22v-5"/><path d="M9 7V2"/><path d="M15 7V2"/><path d="M6 13V8h12v5a5 5 0 0 1-10 0"/>');
        const iconRefresh = icon('<polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/>');
        return `<!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8">
